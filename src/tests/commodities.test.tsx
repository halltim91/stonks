...
import Commodity, { calculateChange, ComodityInfo, CommoditySymbolMap, percentChange } from '../components/comodities/comodities';
...
describe('Commodity component test', () => {
  it('Check for headers', async () => { 
    await loadMockData()  
    const commodityHeader = await screen.findByRole('columnheader', { name: 'Commodity' });
    const valueHeader = await screen.findByRole('columnheader', { name: 'Value' });   
    const ChgAmtHeader = await screen.findByRole('columnheader', { name: 'Chg Amt' });   
    const percentChgHeader = await screen.findByRole('columnheader', { name: '%Chg' }); 
